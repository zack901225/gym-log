...
try {
  ...
  await check('warm-up fields and save', async () => { await nav('today'); await click('＋ 新增熱身項目'); await page.getByRole('button', { name: /^Treadmill Warm-up/ }).click(); await page.getByRole('spinbutton', { name: '速度', exact: true }).fill('6'); await click('✓ 完成熱身'); await page.getByRole('heading', { name: '今天，好好練。' }).waitFor(); const e = (await state()).workouts[0].warmupEntries[0]; assert.equal(e.speed, 6); assert.equal(e.duration, 5); assert.ok(e.completed); });
  ...
  await check('refresh persistence and copy actual clipboard', async () => { await page.reload(); await page.getByRole('heading', { name: '今天，好好練。' }).waitFor(); await click('複製給 ChatGPT ↗'); await waitToast('已複製，可以貼到 ChatGPT'); const text = await page.evaluate(() => navigator.clipboard.readText()); for (const s of ['【熱身】', '【重訓】', '【有氧】', '45 kg × 9', 'RPE 9', '坡度 1%', '椅子高度 4']) assert.ok(text.includes(s), s); await page.screenshot({ path: fileURLToPath(new URL('today-recorded.png', out)), fullPage: true }); });
  await check('history and exercise history', async () => { await nav('history'); await page.locator('[data-action="history"]').first().click(); await page.locator('.entry-link[data-action=exercise-history]').filter({ hasText: /^Chest Press/ }).click(); await page.getByText('最近 10 次訓練').waitFor(); assert.ok((await page.locator('#app').innerText()).includes('45 kg × 9')); });
  let backup;
  await check('JSON export actual download', async () => { await nav('settings'); const download = page.waitForEvent('download'); await click('↓ 匯出 JSON 備份'); backup = JSON.parse(await readFile(await (await download).path(), 'utf8')); assert.equal(backup.schemaVersion, 1); assert.equal(backup.workouts[0].strengthEntries.length, 2); });
  ...
  await check('units / theme', async () => { await page.getByLabel('重量單位').selectOption('lb'); await page.getByLabel('顯示主題').selectOption('light'); assert.equal(await page.locator('html').getAttribute('data-theme'), 'light'); await nav('today'); await page.getByText('99.21 lb × 10 · RPE 9', { exact: true }).waitFor(); await nav('settings'); await page.getByLabel('重量單位').selectOption('kg'); await page.getByLabel('顯示主題').selectOption('dark'); });
